...
//! let mut port = SerialPort::builder("/dev/ttyUSB0", 115200).open()?;
...
impl SerialPort {
    ...
    pub fn open(path: impl AsRef<Path>, baud_rate: u32) -> Result<Self> {
        ...
        new(path.as_os_str().to_string_lossy(), baud_rate).open()
    }

    /// Get a builder for a serial port with the given path and baud_rate.
    pub fn builder(path: impl AsRef<Path>, baud_rate: u32) -> SerialPortBuilder {
        let path = path.as_ref();
        new(path.as_os_str().to_string_lossy(), baud_rate)
    }
