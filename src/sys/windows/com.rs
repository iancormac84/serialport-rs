...
use windows_sys::Win32::Foundation::{
    CloseHandle, DUPLICATE_SAME_ACCESS, DuplicateHandle, ERROR_IO_PENDING, FALSE, GENERIC_READ, GENERIC_WRITE, GetLastError, HANDLE, INVALID_HANDLE_VALUE, TRUE
};
use windows_sys::Win32::Storage::FileSystem::{
    CreateFileW, FILE_ATTRIBUTE_NORMAL, FILE_FLAG_OVERLAPPED, FlushFileBuffers, OPEN_EXISTING, ReadFile, WriteFile
};
use windows_sys::Win32::System::IO::{GetOverlappedResult, OVERLAPPED};
use windows_sys::Win32::System::Threading::GetCurrentProcess;

use super::dcb::{self, DCBBitField};
use super::event_cache::EventCache;
...
impl io::Read for &SerialPort {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if unsafe { GetOverlappedResult(self.handle, &overlapped, &mut len, TRUE) } == FALSE {
            return Err(io::Error::last_os_error());
        }
        match len {
            0 if buf.is_empty() => Ok(0),
            0 => Err(io::Error::new(
...
impl SerialPortExt for SerialPort {
    ...
    fn set_comm_timeouts(&self, timeouts: CommTimeouts) -> Result<()> {
        ...
        if unsafe { SetCommTimeouts(self.handle, &timeouts) } == FALSE {
            return Err(super::error::last_os_error());
        }
        Ok(())
    }
